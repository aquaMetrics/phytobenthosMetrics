...
test_that("Check DARLEQ-TAXON-DICTIONARY is same as DALREQlookup", {
 ...
 # ignore columns - errors due to unicode corruption: 'Authority','DARESAuthority'
  mismatches <- all.equal(DARLEQ_TAXON_DICTIONARY[,-c(6,10)], LookUpDARLEQ2[,-c(6,10)])

  expect_true(mismatches == T)

  # If this is false, check if either file has been changed and sync.
  # Changes should flow from DARLEQ-TAXON-DICTIONARY to LookUpDARLEQ2 - if chnages agreed by all
