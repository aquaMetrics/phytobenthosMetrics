...
CalcDARLEQ.L <- function(data = NULL, version="latest") {
  ...
  na.ltdi2 <- LookUpDARLEQ2[is.na(LookUpDARLEQ2$LTDI2) &
                              LookUpDARLEQ2$TaxonId %in% unique(data$Code),"TaxonId"]
  na.ltdi1 <- LookUpDARLEQ2[is.na(LookUpDARLEQ2$LTDI1) &
                              LookUpDARLEQ2$TaxonId %in% unique(data$Code),"TaxonId"]
  ...
  lake.samples <- as.data.frame(
    ...
    if (VERSP %in% versions) { # older version
      ...
      out$oLTDI1 <- (sum(df$Abundance * df$LTDI1, na.rm=TRUE) /
                       sum(df[!is.na(df$LTDI1), "Abundance"],
                           na.rm=TRUE) * 25) - 25
      ...
      intervals <- cut(out[out$`Lake Alk class` == "LA", "EQR LTDI1"],
                       breaks=c(0, LTDI1class.LA[, "upperlimit"]),
                       include.lowest=TRUE)

      key <- data.frame(range=levels(intervals),
                        rc=rownames(LTDI1class.LA))
      ...
      intervals <- cut(out[out$`Lake Alk class` == "MA", "EQR LTDI1"],
                       breaks=c(0, LTDI1class.MA[, "upperlimit"]),
                       include.lowest=TRUE)

      key <- data.frame(range=levels(intervals),
                        rc=rownames(LTDI1class.MA))
  ...
  if (VERSP %in% versions) {
    ...
    prev.CoC.lakes <- prev.CoC.lakes[c(ncol(prev.CoC.lakes),
                                       1:ncol(prev.CoC.lakes)-1)]

  }

  DARLEQ.L <- switch(version,
                     all = {
                       list(lakes = lake.samples,
